...
resource "aws_eks_node_group" "private-nodes" {
  ...
  #Cluster Autoscaler is configured by AWS
  scaling_config {
    desired_size = 1
    max_size = 5
    min_size = 0
  ...
  launch_template {
    ...
    version = aws_launch_template.eks-with-discks.latest_version
  }

  depends_on = [
    aws_iam_role_policy_attachment.nodes-AmazonEKSNodePolicy,
